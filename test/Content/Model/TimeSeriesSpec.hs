...
spec = do
  ...
  describe "from" $ do
    ...
    it "should create samples of defined points" $ 
      ...
      in from date [status1, status2, status3] `shouldBe` TimeSeries
    ...
    it "should work for edge case of empty status list" $
      from date [] `shouldBe` TimeSeries [] [] [] []
    it "should work for edge case of list of empty status" $
      from date [emptyStatus, emptyStatus, emptyStatus] `shouldBe` TimeSeries [] [] [] []
