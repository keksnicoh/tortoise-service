...
spec = do
  describe "ToJson" $ do
    it "should convert Status into the expected Aeson Value" $
      let
        status = Status
          UUID.nil (Just 2.5) (Just 5) (Just 6) (Just 7) (read "2019-08-31 05:14:37Z")
    ...
    it "should convert a minimal Status into the expected Aeson Value" $
      let
        status = Status
          UUID.nil Nothing Nothing Nothing Nothing (read "2019-08-31 05:14:37Z")
        expectedValue = Object $ fromList
          [ ("statusId",           String "00000000-0000-0000-0000-000000000000")
          , ("temperature",        Null)
          , ("humidity",           Null)
  ...
  describe "from" $
    it "should read properties from core database model" $
      let
        coreStatus = C.Status
          UUID.nil (Just 13) (Just 1.4) Nothing (Just 9) (read "2019-08-31 05:14:37Z")
