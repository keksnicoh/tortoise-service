...
import Content.Model.Monitor
  ( Monitor
      ( Monitor,
        ...
        houseState,
        sensorHumidity,
        sensorHumidityOutside,
        sensorTemperatur,
        sensorTemperaturOutside,
        switchLight1,
        switchLight2,
        weather,
        webcamDate
...
import Content.Model.Switch (Switch (Switch))
import qualified Core.Database.Model.Status as CDMStatus
import qualified Core.OpenWeatherMap.Model.Forecast as COMForecast
...
spec = do
  ...
  describe "from" $ do
    ...
    it "should produce correct Monitor data" $ do
      ...
      from (dates !! 1) [status1, status2, status3, status4] state forecastResult
        `shouldBe` minimalMonitor
          { date = dates !! 1,
    ...
    it "should render a default weather string" $ do
      ...
      from (dates !! 1) [] emptyState forecastResult
        `shouldBe` Monitor
          { date = dates !! 1,
            sensorTemperatur = Nothing,
            sensorHumidity = Nothing,
