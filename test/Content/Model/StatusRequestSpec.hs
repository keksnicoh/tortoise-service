...
spec = do
  describe "FromJson" $ do
    it "should decode a json document properly" $ 
      ...
      in  parse parseJSON value `shouldBe` Success (StatusRequest 2.1 10  (Just 3) (Just 2))
    it "should decode a minimal json document properly" $ 
      ...
      in  parse parseJSON value `shouldBe` Success (StatusRequest 2.1 10 Nothing Nothing)
  describe "toStatus" $ 
    it "should create a Database model" $ 
      let statusRequest = StatusRequest 13.37 42 (Just 4) (Just 8)
          date = read "2019-08-31 05:14:37"
      in  toStatus statusRequest UUID.nil date `shouldBe` C.Status UUID.nil 13.37 42 (Just 4) (Just 8) date 
