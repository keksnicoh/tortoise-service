...
import Control.Monad.Reader (ReaderT (runReaderT), forM_)
import qualified Core.Database.Model.Status as C
import qualified Data.UUID as UUID
import Database.PostgreSQL.Simple (Connection)
import OpenEnv (Provides)
import SpecDatabase (withDatabaseMigrated)
...
mkSpec env = do
  describe "insertStatusRepository" $ do
    ...
    it "should fully insert the record" $
      withDatabaseMigrated env $ \dbEnv ->
        do
          insert dbEnv
